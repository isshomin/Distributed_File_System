...
## 목적 ✒
이 프로젝트의 목적은 GlusterFS를 이용해 세 대의 서버에 분산 스토리지를 구성하고, Docker 컨테이너에서 공유 디렉토리를 마운트함으로써 데이터 복제와 고가용성을 실현하는 것입니다. 서버 하나가 중단되더라도 다른 서버에서 데이터를 계속 사용할 수 있도록 하여, 장애 상황에서도 안정적인 스토리지 서비스를 유지하는 데 중점을 둡니다.
